...
export const getCategories = async (): Promise<Type.ICategorySchema[]> => {
  const db = await getDB();
  const categories = await db.collection(Type.TableName.category).find<Type.ICategorySchema>({ }).sort('createdAt', -1).toArray();
